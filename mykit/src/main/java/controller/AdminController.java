...
public class AdminController {
	...
	public ModelAndView list(Integer sort, HttpSession session) {
		...
		String ht = Integer.toString((userlist.size())*50 + 100);
		String tht = Integer.toString((userlist.size())*50 + 200);
		ht += "px";
		tht += "px";
		mav.addObject("list", userlist);
		mav.addObject("ht", ht);
		mav.addObject("tht", tht);
		return mav;
	}
	...
	private void mailSend(Mail mail) {
		...
		try {
			FileInputStream fis = new FileInputStream
					("C:/webtest/4.jsp/2.back-end/sou/mykit/src/main/resources/mail.properties"); //절대경로
			prop.load(fis);
