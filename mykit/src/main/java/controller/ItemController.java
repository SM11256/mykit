...
public class ItemController {
	...
	public ModelAndView list(String itype, String userpick, String shkey) {
		ModelAndView mav = new ModelAndView();
		if(shkey == null || shkey.trim().equals("")) {
			shkey = null;
		}
		List<Item> itemlist = service.itemList(itype, userpick, shkey);
		int total = 0;
		if(itemlist.size()%4 != 0) {
			total = itemlist.size()/4 + 1;
		}else {
			total = itemlist.size()/4;
		}
		String ht = Integer.toString(total*345);
		String aht = Integer.toString(total*380 + 50);
		ht += "px";
		aht += "px";
		mav.addObject("itemList", itemlist); //jsp에서 itemList로 가져오기 때문에 이름 같아야함(내가 틀린 부분)
		mav.addObject("ht", ht);
		mav.addObject("aht", aht);
		return mav;
	}
